...
export default async function Talks({ params }: { params: { slug: string } }) {
    ...
    return (
        // ... existing code ...
        <div className="legacyPage md:w-full h-screen mt-0"> {/* Adjusted to cover full height */}
            <div className="flex h-full"> {/* Added full height to the flex container */}
                <div className="relative prose max-w-none w-full h-full"> {/* Set width and height to full */}
                    <iframe
                        ...
                        title={title}
                    />
                    <Link href="/talks" className="absolute top-4 left-4 z-10 flex items-center border border-border bg-background px-4 py-2 font-mono text-xs font-medium text-foreground hover:bg-surface">
